...
abstract class TransactionRoomDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         */
        suspend fun populateDatabase(wordDao: TransactionDao) {
            // Start the app with a clean database every time.
            // Not needed if you only populate on creation.
            wordDao.deleteAll()

            var transaction = Transaction(1,"dfd0","summary", "23032", "transaction")
            wordDao.insert(transaction)
            transaction = Transaction(2,"dfd0","summary", "23032", "transaction")
            wordDao.insert(transaction)
        }
    }
}
